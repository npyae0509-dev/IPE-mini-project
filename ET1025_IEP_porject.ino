...
void light_reminder(int ldrVal,float ptemp)
{
  ...
  if(ldrVal>900 && ptemp<=50) //adjust the number of light that should be kept on
  {
    ...
    { 
      ALL_LEDOFF();
      Serial.println(F(""));
      Serial.println(F("The lighting in the room is enought switching off some of the lights."));
      bright=false;
      digitalWrite(LED_BLUE,LOW);
    }
  }
  ...
  if(ldrVal<150 && ptemp<=50) //automatically turn on the lights
  {
    if(dark)
    {
      ALL_LEDOFF();
      Serial.println(F(""));
      Serial.println(F("The room is dark lights are being turn on."));
      dark=false;
      digitalWrite(LED_BLUE,HIGH);
    }
  }
...
void health_Check(int ldrVal,float ptemp,int knobVal,bool &menu,bool &healthMode)
{
  ...
  {
    KEY_POWER, KEY_MENU, KEY_TEST, KEY_PLUS, KEY_BACK,
    KEY_PREV, KEY_PLAY, KEY_NEXT, KEY_ZERO, KEY_MINUS,
    KEY_C, KEY_ONE, KEY_TWO, KEY_THREE, KEY_FOUR,
    KEY_FIVE, KEY_SIX, KEY_SEVEN, KEY_EIGHT, KEY_NINE
  };

  static const char* IR_LABELS[] =
  {
    "POWER", "MENU", "TEST", "PLUS", "BACK",
    "PREV", "PLAY", "NEXT", "ZERO", "MINUS",
    "C", "ONE", "TWO", "THREE", "FOUR",
    "FIVE", "SIX", "SEVEN", "EIGHT", "NINE"
  };

  static const int IR_KEY_COUNT = sizeof(IR_CODES) / sizeof(IR_CODES[0]);

  static bool irOK[IR_KEY_COUNT]={false};
    ...
    case 3:
    {
      ...
      if (IR.decode())
      {
        if (IR.isReleased())
        {
          for(int i=0;i<IR_KEY_COUNT;i++)
          {
            if(IR.keycode==IR_CODES[i] && !irOK[i])
            {
              irOK[i]=true;
              Serial.print(IR_LABELS[i]);
              Serial.println(F(": OK"));
              break;
            }
          }
